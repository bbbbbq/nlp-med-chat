...
"""
    ...
    def chat_with_diagnosis_api(self, prompt):
        ...
        data = {
            ...
            "max_tokens": 1500,  # 减少最大token数，避免冗余内容
            "temperature": 0.3,   # 适度提高创造性，避免重复
            "top_p": 0.9,        # 核采样，提高输出质量
            "frequency_penalty": 0.6,  # 频率惩罚，减少重复
            "presence_penalty": 0.3,   # 存在惩罚，鼓励多样性
            "stop": ["参考资料：", "《", "祝您"]  # 停止词，避免重复参考资料
        }
...
**请基于症状和医学知识参考，按以下格式提供诊断建议：**

【可能诊断】：
1. 诊断可能性1（概率：高/中/低）- 基于知识库匹配度 {max_relevance_score:.3f}
2. 诊断可能性2（概率：高/中/低）- 基于知识库匹配度
...
...
【知识库匹配分析】：
- 症状与知识库中疾病的匹配程度分析（最高匹配度: {max_relevance_score:.3f}）
- 需要排除的疾病
